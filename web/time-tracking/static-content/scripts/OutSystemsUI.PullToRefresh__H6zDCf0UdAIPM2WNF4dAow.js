...
var WebPullToRefresh = (function () {
    ...
    var _touchMove = function(e) {
        ...
        /* when we are on the phone with a Master-Detail opened or 
           if we have a Master-Detail and we are targeting an element inside and not at the top 
           we won't trigger this
         */
        let closesteSelect = e.target.closest('.split-left, .split-right');
        if(document.querySelector('.phone .split-right.open') || (closesteSelect && closesteSelect.scrollTop !== 0))
        {
            return;
        }

        if(document.querySelector('.layout-native')) {
            if(document.querySelector('.screen-container').scrollTop === 0) {
                var touchobj = e.changedTouches[0];
    ...
    var _panEnd = function(e) {
        ...
        var strStyle = document.querySelector('.main-content').getAttribute('style');
